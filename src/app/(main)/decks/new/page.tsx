...
export function CreateDeckForm() {
  ...
  const handleImportComplete = () => {
    if (createDeck.data?.deck.id) {
      router.push(`/decks/${createDeck.data.deck.id}/edit`);
    }
  };

  // Handle from scratch form submit
  const onSubmit = (data: CreateDeck) => {
    ...
    createDeck.mutate(
      {
        ...
        commanderScryfallCard: selectedCommander,
        basicLands: pendingBasicLands ?? undefined,
      },
      {
        onSuccess: (result) => router.push(`/decks/${result.deck.id}/edit`),
        onError: () => toast('Failed to create deck. Please try again.', 'error'),
      }
    );
  };
